...
function plistValue(plistPath, key) {
  return runCommand(command("plutil", ["-extract", key, "raw", "-o", "-", plistPath]), {
    cwd: paths.repositoryRoot,
    capture: true,
  }).stdout.trim();
}
...
export function verifyMacAppArtifact(appPath, variant) {
  const contents = path.join(appPath, "Contents");
  const plist = path.join(contents, "Info.plist");
  if (plistValue(plist, "CFBundleName") !== productInfo.name) throw new Error(`${variant} app has an invalid product name`);
  if (plistValue(plist, "CFBundleShortVersionString") !== productInfo.version) throw new Error(`${variant} app has an invalid marketing version`);

  const iconName = plistValue(plist, "CFBundleIconFile");
  const iconPath = path.join(contents, "Resources", iconName.endsWith(".icns") ? iconName : `${iconName}.icns`);
  const icon = fs.readFileSync(iconPath);
  if (icon.subarray(0, 4).toString("ascii") !== "icns" || icon.length < 100_000) throw new Error(`${variant} app icon is missing or invalid`);

  const executable = path.join(contents, "MacOS", plistValue(plist, "CFBundleExecutable"));
  assertExecutable(executable, `${variant} app executable`);
  if (variant === "swift") {
    if (plistValue(plist, "KawaCADBuildChannel") !== "release") throw new Error("Swift app is not marked as release");
    if (plistValue(plist, "NSHumanReadableCopyright") !== productInfo.copyright) throw new Error("Swift app has invalid copyright metadata");
    const notice = filesBelow(contents).find((item) => item.endsWith("ThirdPartyNotices.generated.md"));
    if (!notice) throw new Error("Swift app does not contain generated third-party notices");
    const text = fs.readFileSync(notice, "utf8");
    if (!text.startsWith("# Third-party notices") || /Generated by scripts|^## .*tauri/imu.test(text)) throw new Error("Swift app contains invalid third-party notices");
  } else if (!fs.readFileSync(executable).includes(Buffer.from("/ThirdPartyNotices.json"))) {
    throw new Error("Tauri app does not embed its offline third-party notices");
  }
}
...
function stageSwiftApp({ dryRun = false, env = {} } = {}) {
  ...
  verifyMacAppArtifact(destination, "swift");
...
function stageTauriMacApp({ dryRun = false } = {}) {
  ...
  verifyMacAppArtifact(destination, "tauri");
  console.log(`[artifact] ${destination}`);
}
