...
Windows adapter は GDI/DEVMODE を用いる。`DeviceCapabilitiesW` で対応用紙とドライバーが扱う DEVMODE 項目を照会し、対応項目に限って `DocumentPropertiesW` で A4、`dmScale = 100`、片面、N-up 無効を正規化してから、`CreateDCW` と `GetDeviceCaps` で印刷可能領域を求める。`DC_DUPLEX == 0` は片面専用を表すため拒否理由にせず、`DM_DUPLEX` や `DM_NUP` を扱わないドライバーにもそれらのフィールド保持を要求しない。実行時は同じ正規化結果を再確認し、`PrintRenderData` を 1 mm = DPI / 25.4 device unit で描画する。
